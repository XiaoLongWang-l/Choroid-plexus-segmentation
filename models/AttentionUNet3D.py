...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.encoder_block = nn.Sequential(nn.Conv3d(in_channels, out_channels, 3, padding=1, bias=False),
                                           nn.BatchNorm3d(num_features=out_channels),
                                           nn.ReLU(inplace=True),
                                           nn.Conv3d(out_channels, out_channels, 3, padding=1, bias=False),
                                           nn.BatchNorm3d(num_features=out_channels),
                                           nn.ReLU(inplace=True),
                                           # 原论文中是每层有两个卷积核进行下采样https://arxiv.org/abs/1505.04597
                                           nn.Conv3d(out_channels, out_channels, 3, padding=1, bias=False),
                                           nn.BatchNorm3d(num_features=out_channels),
                                           nn.ReLU(inplace=True))
...
class CenterBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.center_block = nn.Sequential(nn.Conv3d(in_channels, mid_channels, 3, padding=1, bias=False),
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        mid_channels = int(in_channels / 2)
        self.decoder_block = nn.Sequential(nn.Conv3d(in_channels, mid_channels, 3, padding=1, bias=False),
                                           nn.BatchNorm3d(num_features=mid_channels),
                                           nn.ReLU(inplace=True),
...
class FinalBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        mid_channels = int(in_channels / 2)
        self.final_block = nn.Sequential(nn.Conv3d(in_channels, mid_channels, 3, padding=1, bias=False),
                                         nn.BatchNorm3d(num_features=mid_channels),
                                         nn.ReLU(inplace=True),
                                         nn.Conv3d(mid_channels, mid_channels, 3, padding=1, bias=False),
                                         nn.BatchNorm3d(num_features=mid_channels),
...
class AttentionGates3D(nn.Module):
    def __init__(self, F_g, F_l, F_int):
        ...
        self.W_g = nn.Sequential(nn.Conv3d(F_l, F_int, kernel_size=1, stride=1, padding=0, bias=True),
                                 nn.BatchNorm3d(F_int))
        self.W_x = nn.Sequential(nn.Conv3d(F_g, F_int, kernel_size=1, stride=1, padding=0, bias=True),
                                 nn.BatchNorm3d(F_int))
        self.psi = nn.Sequential(nn.Conv3d(F_int, 1, kernel_size=1, stride=1, padding=0, bias=True),
...
if __name__ == '__main__':
    data_ = torch.rand(1, 1, 16, 128, 128)
